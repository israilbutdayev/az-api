...
export async function print(data) {
    ...
    if (fit) {
        ...
        outer: for (let j = 1; j <= 10; j++) {
            ...
            const asc_index = sorted_asc.findIndex((v) => v.scale >= scale_initial && v.page_num > page_num_initial);
            const desc_index = sorted_desc.findIndex((v) => v.scale < scale_initial && v.page_num < page_num_initial);
            let max;
            let min;
            if (asc_index >= 0) {
